...
export default function ModalDetail({ show, onHide, asset, onEdit, onDelete }) {
  ...
  const formatValue = (key, value) => {
    ...
    if (
      ...
      key.toLowerCase() === 'current_status' ||
      key.toLowerCase() === 'status_history'
    ) {
      const isActive = value === 'Active' || value === 'Aktif';
      return <Badge className={isActive ? 'badge-active' : 'badge-inactive'}>{value}</Badge>;
    }
    ...
    if (
      ...
      key === 'depreciation_end_date' ||
      key === 'extend_warranty_date'
    ) {
      if (
