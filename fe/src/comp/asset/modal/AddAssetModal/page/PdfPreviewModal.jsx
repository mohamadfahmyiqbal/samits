...
// resolveDocumentUrl dipindah ke utils atau AssetFormModal jika diperlukan
// Untuk sekarang, definisikan di sini
export const resolveDocumentUrl = (fileUrl) => {
  if (!fileUrl) return "";
  if (/^(https?:)?\/\//i.test(fileUrl) || fileUrl.startsWith("blob:") || fileUrl.startsWith("data:")) {
    return fileUrl;
  }
  const API_ORIGIN = process.env.REACT_APP_API_BASE_URL?.replace(/\/api\/?$/, "").replace(/\/+$/, "") || "http://localhost:5002";
      ...
      <Modal.Body style={{ height: '80vh', padding: 0 }}>
        {fileUrl ? (
          ...
          <div className="d-flex justify-content-center align-items-center h-100">
            <p className="text-muted">Tidak dapat memuat pratinjau dokumen.</p>
          </div>
        )}
      </Modal.Body>
    </Modal>
