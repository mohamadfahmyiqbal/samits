...
import { FaDesktop, FaSave, FaTimes, FaPaperclip, FaServer, FaUser, FaCalendarAlt, FaFileAlt, FaTrash, FaEye, FaFilePdf } from "react-icons/fa";
import userService from "../../../../../services/UserService";
import { fetchClassifications, fetchAssetGroups, fetchMainTypes, fetchCategoryTypes, getAssetDetails, deleteAssetDocument, fetchSubCategoryDetails } from "../../../../../services/AssetService";
...
// Remove duplicate local definitions - using imports from assetConstants
...
const [deletingDocId, setDeletingDocId] = useState(null);
  ...
  tahunBeli: latestData.tahunBeli || "",
        ...
        if (asset) {
          const purchasePrice =
            asset.purchase_price_actual ??
            asset.purchase_price ??
            asset.purchase_price_plan ??
            "";
          setFormData({
    ...
    const loadInitialSubCategoryOptions = async () => {
      ...
      try {
        ...
        if (formData.asset_main_type_id && formData.category_id) {
          const response = await fetchCategoryTypes(null, formData.asset_main_type_id, formData.category_id);
        ...
        if (data) {
          ...
          if (data.asset_main_type_id && data.it_category_id) {
            const catResponse = await fetchCategoryTypes(null, data.asset_main_type_id, data.it_category_id);
  ...
  }, [show, formData.sub_category_id, formData.asset_main_type_id, formData.category_id, subCategoryInitialLoaded]);
  ...
  useEffect(() => {
    ...
    const ensureSubCategoryHierarchy = async () => {
      ...
      if (subCatId && (!formData.asset_main_type_id || !formData.category_id) && !loadingSubCategory) {
        setLoadingSubCategory(true);
  ...
  }, [show, formData.sub_category_id, formData.asset_main_type_id, formData.category_id, hierarchyLoaded, loadingSubCategory]);
  ...
  useEffect(() => {
    ...
    const fetchAssetGroupOptions = async () => {
      try {
        const response = await fetchAssetGroups(
          null,
          null,
  ...
  const handleSubmit = async () => {
    if (!formData.noAsset || !formData.nama || !formData.asset_main_type_id || !formData.category || !formData.sub_category) {
          ...
          <Card className="mb-4 border-0 shadow-sm">
            <Card.Body>
              ...
              <Row className="g-3">
                ...
                <Col md={6}>
                  <Form.Group>
                    <Form.Label>Main Type <span className="text-danger">*</span></Form.Label>
                    <Select
                      options={mainTypeOptions}
                      value={mainTypeOptions.find((o) => String(o.value) === String(formData.asset_main_type_id || "")) || null}
                ...
                <Col md={6}>
                  <Form.Group>
                    ...
                    <Select
                      ...
                      value={
                        effectiveCategoryOptions.find((o) => String(o.category_id || o.value || "") === String(formData.category_id || "")) ||
                        effectiveCategoryOptions.find((o) => String(o.label || "").toLowerCase() === String(formData.category || "").toLowerCase()) ||
                        null
                      }
                      onChange={(opt) => handleSelectChange("category", opt)}
                      placeholder={!formData.asset_main_type_id ? "Pilih Main Type dulu" : (effectiveCategoryOptions.length === 0 ? "Loading..." : "Pilih Kategori")}
                      isDisabled={!formData.asset_main_type_id || effectiveCategoryOptions.length === 0}
                      styles={customSelectStyles}
                    />
                  </Form.Group>
                </Col>
                ...
                <Col md={6}>
                  <Form.Group>
                    ...
                    <Select
                      ...
                      value={
                        effectiveSubCategoryOptions.find((o) => String(o.sub_category_id || o.value || "") === String(formData.sub_category_id || "")) ||
                        effectiveSubCategoryOptions.find((o) => String(o.label || "").toLowerCase() === String(formData.sub_category || "").toLowerCase()) ||
                        null
                      }
                      onChange={(opt) => handleSelectChange("sub_category", opt)}
                      placeholder={effectiveSubCategoryOptions.length === 0 ? (loadingSubCategory ? "Loading..." : "Tidak ada data") : "Pilih Sub Kategori"}
                  ...
                  <Col md={6}>
                    <Form.Group>
                      ...
                      <Select
                        ...
                        value={classificationOptions.find((o) => o.value === formData.classification_id)}
                    ...
                    <Select
                      ...
                      value={
                        assetGroupOptions.find(
                          (o) => String(o.asset_group_id || "") === String(formData.asset_group_id || "")
                        ) ||
                        assetGroupOptions.find(
                          (o) =>
                            String(o.label || "").toLowerCase() === String(formData.assetGroup || "").toLowerCase()
                        ) ||
                        null
                      }
                ...
                <Col md={6}>
                  <Form.Group>
                    ...
                    <Form.Control
                      ...
                      readOnly
                      className={`bg-light ${
                        !formData.dept ? 'border-warning' : 'border-success'
                      }`}
                      title={
                        !formData.dept 
                ...
                {existingDocuments.length > 0 && (
                  <div className="mt-2">
                    ...
                    {existingDocuments.map((doc, index) => (
                      <div key={`existing-${index}`} className="d-flex align-items-center justify-content-between p-2 mb-2 bg-light rounded">
                        <div className="d-flex align-items-center">
                          <FaFilePdf className="text-danger me-2" />
                          <span className="text-primary small text-truncate" style={{ maxWidth: '200px' }}>
                ...
                {attachments.length > 0 && (
                  <div className="mt-2">
                    ...
                    {attachments.map((file, index) => (
                      <div key={index} className="d-flex align-items-center justify-content-between p-2 mb-2 bg-light rounded">
