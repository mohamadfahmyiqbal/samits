...
import { Form, Card, Row, Col, Table, Button, Tag, Space, message, Input, Select, Modal, Alert } from 'antd';
import { 
  ReloadOutlined,
  PlusOutlined,
  WarningOutlined
  ...
  const columns = [
      ...
      render: (category) => <Tag color="blue">{category}</Tag> },
    {
      ...
      render: (current, record) => (
        <div>
          <div>
            <strong>{current} / {record.minimum_stock} {record.unit}</strong>
          </div>
          <div style={{ fontSize: '12px', color: '#8c8c8c' }}>
            Shortage: <span style={{ color: '#ff4d4f' }}>{record.shortage} {record.unit}</span>
          </div>
        </div>
      ) },
    {
        ...
        {selectedItem && (
          ...
          >
            ...
            >
              <Input.TextArea 
                rows={3} 
                placeholder="Catatan untuk penambahan stock..." 
              />
            </Form.Group>
