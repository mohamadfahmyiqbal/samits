...
export default function ApprovalSystem() {
    ...
    setApprovalData([
      {
        ...
        can_approve: true,
        approval_flow: [
          { level: 1, role: 'Supervisor', name: 'Alice Johnson', status: 'pending', action_date: null },
          { level: 2, role: 'Manager', name: 'Bob Wilson', status: 'pending', action_date: null },
          { level: 3, role: 'Director', name: 'Charlie Brown', status: 'pending', action_date: null }
        ],
        details: {
      ...
      },
      {
        ...
        can_approve: true,
        approval_flow: [
          { level: 1, role: 'Supervisor', name: 'Alice Johnson', status: 'approved', action_date: '2024-03-24', comments: 'Approved for disposal' },
          { level: 2, role: 'Manager', name: 'Bob Wilson', status: 'pending', action_date: null },
          { level: 3, role: 'Finance', name: 'Diana Prince', status: 'pending', action_date: null },
          { level: 4, role: 'Director', name: 'Charlie Brown', status: 'pending', action_date: null }
        ],
        details: {
          assets_count: 5,
      ...
      },
      {
        ...
        can_approve: false,
        approval_flow: [
          { level: 1, role: 'Supervisor', name: 'Alice Johnson', status: 'approved', action_date: '2024-03-23', comments: 'Approved for transfer' },
          { level: 2, role: 'Manager', name: 'Bob Wilson', status: 'approved', action_date: '2024-03-23', comments: 'Transfer approved' },
          { level: 3, role: 'Director', name: 'Charlie Brown', status: 'approved', action_date: '2024-03-24', comments: 'Final approval granted' }
        ],
        details: {
          assets_count: 3,
  ...
  const handleSubmitApproval = async (values) => {
    try {
      ...
      // Update local state
      setApprovalData(prev => prev.map(item => {
        if (item.id === selectedApproval.id) {
          const updatedFlow = [...item.approval_flow];
          const currentLevelIndex = updatedFlow.findIndex(flow => flow.level === item.current_level);
          ...
          const newStatus = values.action === 'approve' 
            ? (item.current_level === item.total_levels ? 'completed' : 'pending')
            : 'rejected';
          const newLevel = values.action === 'approve' && item.current_level < item.total_levels 
          ...
          return {
            ...
            can_approve: values.action === 'approve' && newStatus === 'pending' && newLevel <= item.total_levels
          };
        }
        return item;
      }));
  ...
  const getApprovalSteps = (approvalFlow, currentLevel) => {
    return approvalFlow.map((step, index) => ({
      ...
      status: step.status === 'approved' ? 'finish' : 
              ...
              step.level === currentLevel ? 'process' : 'wait',
      icon: step.status === 'approved' ? <CheckCircleOutlined /> : 
    ...
    <div className="approval-system">
      ...
      <Card>
        <div className="tab-navigation">
          ...
          >
            Disetujui ({approvalData.filter(d => d.status === 'approved' || d.status === 'completed').length})
          </Button>
          <Button 
          ...
          <div className="approval-detail">
            <Row gutter={[16, 16]}>
              <Col span={12}>
                ...
                <p><strong>Request ID:</strong> {selectedApproval.request_id}</p>
                <p><strong>Tipe:</strong> <Tag color="blue">{selectedApproval.request_type}</Tag></p>
                <p><strong>Judul:</strong> {selectedApproval.title}</p>
                <p><strong>Pemohon:</strong> {selectedApproval.requester_name}</p>
                <p><strong>Status:</strong> <Tag color="orange">{selectedApproval.status}</Tag></p>
              </Col>
              <Col span={12}>
                <h4>Progress Approval</h4>
                <p><strong>Level Saat Ini:</strong> {selectedApproval.current_level}/{selectedApproval.total_levels}</p>
                <div className="approval-steps">
                  <Steps
                    current={selectedApproval.current_level - 1}
                    items={getApprovalSteps(selectedApproval.approval_flow, selectedApproval.current_level)}
                    size="small"
                  />
                </div>
              </Col>
            ...
            <div className="request-details">
              ...
              {selectedApproval.request_type === 'asset_request' && (
                <Row gutter={16}>
                  <Col span={12}>
                    <p><strong>Nama Aset:</strong> {selectedApproval.details.asset_name}</p>
                    <p><strong>Jumlah:</strong> {selectedApproval.details.quantity}</p>
                  </Col>
                  <Col span={12}>
                    <p><strong>Estimasi Biaya:</strong> Rp {selectedApproval.details.estimated_cost.toLocaleString('id-ID')}</p>
                    <p><strong>Justifikasi:</strong> {selectedApproval.details.justification}</p>
                  </Col>
                </Row>
              )}
              {selectedApproval.request_type === 'disposal' && (
                <Row gutter={16}>
                  <Col span={12}>
                    <p><strong>Jumlah Aset:</strong> {selectedApproval.details.assets_count}</p>
                    <p><strong>Tipe Disposal:</strong> {selectedApproval.details.disposal_type}</p>
                  </Col>
                  <Col span={12}>
                    <p><strong>Total Depresiasi:</strong> Rp {selectedApproval.details.total_depreciation.toLocaleString('id-ID')}</p>
                    <p><strong>Justifikasi:</strong> {selectedApproval.details.justification}</p>
                  </Col>
                </Row>
              )}
              {selectedApproval.request_type === 'transfer' && (
                <Row gutter={16}>
                  <Col span={12}>
                    <p><strong>Jumlah Aset:</strong> {selectedApproval.details.assets_count}</p>
                    <p><strong>Dari:</strong> {selectedApproval.details.from_location}</p>
                  </Col>
                  <Col span={12}>
                    <p><strong>Ke:</strong> {selectedApproval.details.to_location}</p>
                    <p><strong>Justifikasi:</strong> {selectedApproval.details.justification}</p>
                  </Col>
                </Row>
              )}
                ...
                {selectedApproval.approval_flow.map((step, index) => (
                  <Timeline.Item
                    ...
                    color={step.status === 'approved' ? 'green' : 
                           ...
                           step.level === selectedApproval.current_level ? 'blue' : 'gray'}
                  ...
                  >
                    <div className="timeline-content">
                      <div className="timeline-header">
                        ...
                        <Tag color={step.status === 'approved' ? 'green' : 
                                   step.status === 'rejected' ? 'red' : 'orange'}>
                          {step.status}
                        </Tag>
                      </div>
                      <p><strong>Nama:</strong> {step.name}</p>
                      {step.action_date && <p><strong>Tanggal:</strong> {step.action_date}</p>}
                      {step.comments && <p><strong>Catatan:</strong> {step.comments}</p>}
                    </div>
                  </Timeline.Item>
                ))}
              </Timeline>
