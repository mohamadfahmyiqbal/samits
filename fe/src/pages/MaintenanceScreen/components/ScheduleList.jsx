...
import { Card, Button, Tooltip, OverlayTrigger, Badge } from 'react-bootstrap';
      ...
      <Card className='border-0 shadow-sm h-100'>
        ...
        <Card.Body className='d-flex flex-column justify-content-center align-items-center text-center p-5'>
          ...
          <p className='text-muted px-4'>
            Klik pada tanggal di kalender untuk melihat detail jadwal maintenance
          </p>
        </Card.Body>
      </Card>
    );
  ...
  return (
    <Card className='border-0 shadow-sm h-100'>
      <Card.Header className='bg-white border-bottom py-3 d-flex justify-content-between align-items-center'>
        ...
        <Badge bg='secondary' pill className='px-2'>
          {selectedDateLogs.length} Jadwal
        </Badge>
      </Card.Header>
        ...
        {selectedDateLogs.length === 0 ? (
          ...
          <div className='d-flex flex-column gap-3'>
            {selectedDateLogs.map((log, idx) => (
              <Card
                key={log.id || log.itItemId || idx}
                className='border-0 shadow-sm hover-border transition-all'
              >
