  ...
  return (
    <Modal show={show} onHide={onHide} size='lg'>
      ...
      <Modal.Body>
        <Row>
          <Col md={6}>
            <strong>Asset:</strong> {task.assetName || task.assetId}
            <br />
            ...
            <br />
            <strong>Tanggal Jatuh Tempo:</strong> {task.dueDate}
            <br />
            <strong>Dipilih:</strong> {task.assignedTo}
            <br />
          </Col>
          <Col md={6}>
          ...
          {task.checklist?.map((item, idx) => (
            <div key={idx} className='form-check mb-1'>
              ...
              <label className='form-check-label'>
                {item.description}{' '}
                {item.completed && <i className='bi bi-check-circle text-success ms-1'></i>}
              </label>
            </div>
...
export default function PMTask() {
  ...
  return (
    <div className='p-4'>
      <Row className='mb-4'>
        ...
        <Col md={4} className='text-end'>
          ...
          <Badge bg='warning' className='py-2 px-3'>
            {
              filteredTasks.filter((t) => t.status === 'pending' || t.status === 'in_progress')
                .length
            }{' '}
            Pending
          </Badge>
                ...
                <tbody>
                  {filteredTasks.map((task) => {
                    ...
                    const progress = Math.round(
                      (task.completedSubtasks / task.totalSubtasks || 0) * 100
                    );
      ...
      <TaskDetailModal task={selectedTask} show={showModal} onHide={() => setShowModal(false)} />
    </div>
  );
}
