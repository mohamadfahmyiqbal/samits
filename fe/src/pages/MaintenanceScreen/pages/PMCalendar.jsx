...
const PMEvent = ({ event }) => (
  <div className="event-wrapper">
    ...
    {event.status === 'overdue' && (
      <Badge bg="danger" className="ms-1">Terlambat</Badge>
    )}
        ...
        <div className="calendar-wrapper" style={{ height: "100%" }}>
          <Calendar
            ...
            eventPropGetter={(event) => ({
              ...
              style: { borderLeft: `4px solid var(--bs-${event.status === 'overdue' ? 'danger' : event.status === 'completed' ? 'success' : 'info'})` }
            })}
            components={{
              event: PMEvent,
            ...
            <div>
              <Row>
                <Col md={6}>
                  ...
                  <h6><strong>PIC:</strong></h6>
                  <p>{selectedEvent.pic}</p>
                </Col>
                <Col md={6}>
                  <h6><strong>Status:</strong></h6>
                  <Badge bg={selectedEvent.status === 'overdue' ? 'danger' : 
                            selectedEvent.status === 'completed' ? 'success' : 'warning'}>
