...
import { Form, Card, Table, Button, Input, Select, Row, Col, message, Modal, Alert, Tag, Upload, Divider, Typography } from 'antd';
import { 
  FileTextOutlined, 
  PlusOutlined, 
  EyeOutlined, 
...
export default function MeetingMinutes() {
      ...
      discussions: [
        {
          ...
          summary: 'Revenue increased by 15% compared to Q1 2023. Operating costs reduced by 8% through efficiency improvements.',
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (editingMinutes) {
        setMinutesData(prev => prev.map(item => 
          item.id === editingMinutes.id ? newMinutes : item
        ));
          ...
          <Card>
            <div className="statistic-card draft">
              ...
              <div className="statistic-content">
                ...
                <div className="statistic-value">{minutesData.filter(m => m.status === 'draft').length}</div>
              </div>
            </div>
          </Card>
        </Col>
          ...
          <Card>
            <div className="statistic-card approved">
              ...
              <div className="statistic-content">
                ...
                <div className="statistic-value">{minutesData.filter(m => m.status === 'approved').length}</div>
              </div>
            </div>
          </Card>
        </Col>
          ...
          <Card>
            <div className="statistic-card">
              ...
              <div className="statistic-content">
                ...
                <div className="statistic-value">{minutesData.reduce((sum, m) => sum + (m.attendees?.length || 0), 0)}</div>
              </div>
            </div>
          </Card>
        </Col>
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (total, range) => 
              `${range[0]}-${range[1]} dari ${total} minutes` }}
        />
      </Card>
        ...
        {selectedMinutes && (
          <div className="minutes-detail">
            <div className="minutes-header">
              ...
              <Paragraph><strong>Meeting Type:</strong> {selectedMinutes.meetingType}</Paragraph>
              <Paragraph><strong>Date:</strong> {selectedMinutes.meetingDate} at {selectedMinutes.meetingTime}</Paragraph>
              <Paragraph><strong>Location:</strong> {selectedMinutes.location}</Paragraph>
              <Divider />
              <Paragraph><strong>Chairman:</strong> {selectedMinutes.chairman}</Paragraph>
              <Paragraph><strong>Secretary:</strong> {selectedMinutes.secretary}</Paragraph>
              <Paragraph><strong>Status:</strong> <Tag color={getStatusColor(selectedMinutes.status)}>{selectedMinutes.status.toUpperCase()}</Tag></Paragraph>
            </div>
            ...
            <div className="minutes-section">
              ...
              {selectedMinutes.discussions.map((discussion, index) => (
                <div key={index} className="discussion-item">
                  ...
                  <Paragraph><strong>Presenter:</strong> {discussion.presenter}</Paragraph>
                  <Paragraph>{discussion.summary}</Paragraph>
                  <Paragraph><strong>Decisions:</strong> {discussion.decisions}</Paragraph>
                </div>
              ))}
            </div>
            ...
            <div className="minutes-section">
              ...
              {selectedMinutes.decisions.map((decision, index) => (
                <div key={index} className="decision-item">
                  ...
                  <Paragraph><strong>Responsible:</strong> {decision.responsible}</Paragraph>
                  <Paragraph><strong>Deadline:</strong> {decision.deadline}</Paragraph>
                </div>
              ))}
            </div>
            ...
            <div className="minutes-section">
              ...
              {selectedMinutes.actionItems.map((action, index) => (
                <div key={index} className="action-item">
                  <Paragraph><strong>• {action.item}</strong></Paragraph>
                  <Paragraph><strong>Assignee:</strong> {action.assignee}</Paragraph>
                  <Paragraph><strong>Deadline:</strong> {action.deadline}</Paragraph>
                  <Paragraph><strong>Status:</strong> <Tag color={action.status === 'completed' ? 'green' : 'orange'}>{action.status.toUpperCase()}</Tag></Paragraph>
                </div>
              ))}
            </div>
