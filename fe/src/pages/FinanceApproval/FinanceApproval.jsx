...
import { Form, Card, Row, Col, Table, Button, Tag, Space, message, Input, Select, Modal, Steps, InputNumber } from 'antd';
import { 
  ReloadOutlined,
  EyeOutlined,
  DollarOutlined,
...
export default function FinanceApproval() {
  ...
  const mockApprovalData = [
    {
      ...
      workflow: [
        { step: 'user_approval', status: 'completed', assignee: 'Department Head', completed_date: '2024-03-26' },
        { step: 'finance_approval', status: 'pending', assignee: 'Finance Manager', completed_date: null },
    ...
    },
    {
      ...
      workflow: [
        { step: 'user_approval', status: 'completed', assignee: 'Finance Department Head', completed_date: '2024-03-27' },
        { step: 'finance_approval', status: 'pending', assignee: 'Finance Manager', completed_date: null },
        { step: 'pd_approval', status: 'pending', assignee: 'Director', completed_date: null }
      ]
    },
    {
      ...
      attachments: ['writeoff_form.pdf', 'technical_assessment.pdf', 'depreciation_calculation.pdf'],
      user_approval: {
      ...
      workflow: [
        { step: 'user_approval', status: 'completed', assignee: 'Operations Head', completed_date: '2024-03-25' },
        { step: 'finance_approval', status: 'completed', assignee: 'Finance Manager', completed_date: '2024-03-28' },
        { step: 'pd_approval', status: 'pending', assignee: 'Director', completed_date: null }
      ]
    }
  ];
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      setApprovalData(prev => prev.map(request => {
        if (request.id === id) {
          const currentStepIndex = request.workflow.findIndex(step => step.step === request.current_step);
  ...
  const pendingRequests = filteredData.filter(request => request.status === 'pending' && request.current_step === 'finance_approval').length;
  ...
  const totalFinancialImpact = filteredData.reduce((sum, request) => sum + (request.financial_impact.loss_amount || 0), 0);
  ...
  const columns = [
      ...
      ellipsis: true },
    {
      ...
      render: (_, record) => (
        <div>
          <div><strong>{record.asset_details.asset_name}</strong></div>
    ...
    <div className="finance-approval">
          ...
          <Card>
            <div className="statistic-card impact">
              ...
              <div className="statistic-content">
                ...
                <div className="statistic-value">Rp {totalFinancialImpact.toLocaleString('id-ID')}</div>
              </div>
            </div>
          </Card>
        </Col>
        ...
        <Col span={24}>
          <Card title="Daftar Finance Approval Request">
            ...
            <Table
              ...
              rowClassName={(record) => {
                if (record.status === 'pending' && record.current_step === 'finance_approval') return 'row-pending';
      ...
      <Modal
        ...
        footer={[
          ...
          selectedRequest?.status === 'pending' && selectedRequest?.current_step === 'finance_approval' && (
            <>
              <Button
                key="approve"
                type="primary"
            ...
            <Row gutter={[16, 16]}>
              <Col span={12}>
                <div className="detail-section">
                  ...
                  <p><strong>Status:</strong> <Tag color={getStatusColor(selectedRequest.status)}>{selectedRequest.status.replace('_', ' ').toUpperCase()}</Tag></p>
            ...
            <Row gutter={[16, 16]}>
              <Col span={24}>
                <div className="detail-section">
                  ...
                  <Row gutter={[16, 16]}>
                    <Col span={8}>
                      <p><strong>Asset Code:</strong> {selectedRequest.asset_details.asset_code}</p>
                      <p><strong>Asset Name:</strong> {selectedRequest.asset_details.asset_name}</p>
                      <p><strong>Purchase Value:</strong> Rp {selectedRequest.asset_details.purchase_value.toLocaleString('id-ID')}</p>
                      <p><strong>Purchase Date:</strong> {selectedRequest.asset_details.purchase_date}</p>
                    </Col>
                    <Col span={8}>
                      <p><strong>Depreciation Years:</strong> {selectedRequest.asset_details.depreciation_years}</p>
                      <p><strong>Accumulated Depreciation:</strong> Rp {selectedRequest.asset_details.accumulated_depreciation.toLocaleString('id-ID')}</p>
                      <p><strong>Book Value:</strong> <span style={{ color: '#1890ff', fontWeight: 'bold' }}>Rp {selectedRequest.asset_details.book_value.toLocaleString('id-ID')}</span></p>
                    </Col>
                    <Col span={8}>
                      <p><strong>Current Market Value:</strong> Rp {(selectedRequest.asset_details.current_value || 0).toLocaleString('id-ID')}</p>
                      <p><strong>Depreciation Method:</strong> Straight Line</p>
                      <p><strong>Annual Depreciation:</strong> Rp {(selectedRequest.asset_details.purchase_value / selectedRequest.asset_details.depreciation_years).toLocaleString('id-ID')}</p>
                    </Col>
                  </Row>
                </div>
              </Col>
                  ...
                  <Row gutter={[16, 16]}>
                    <Col span={8}>
                      <Card size="small" title="Gain/Loss Calculation">
                        <p><strong>Book Value:</strong> Rp {selectedRequest.asset_details.book_value.toLocaleString('id-ID')}</p>
                        <p><strong>Disposal/Transfer Value:</strong> Rp {(selectedRequest.financial_impact.disposal_value || 0).toLocaleString('id-ID')}</p>
                        <p><strong>Loss Amount:</strong> <span style={{ color: '#ff4d4f', fontWeight: 'bold' }}>Rp {(selectedRequest.financial_impact.loss_amount || 0).toLocaleString('id-ID')}</span></p>
                      </Card>
                    </Col>
                    <Col span={8}>
                      <Card size="small" title="Tax Impact">
                        <p><strong>Tax on Gain/Loss:</strong> Rp {(selectedRequest.financial_impact.tax_impact || 0).toLocaleString('id-ID')}</p>
                        <p><strong>Tax Rate:</strong> 10%</p>
                        <p><strong>Net Tax Impact:</strong> <span style={{ color: '#ff4d4f', fontWeight: 'bold' }}>Rp {(selectedRequest.financial_impact.tax_impact || 0).toLocaleString('id-ID')}</span></p>
                      </Card>
                    </Col>
                    <Col span={8}>
                      <Card size="small" title="Net Impact">
                        <p><strong>Administrative Cost:</strong> Rp {(selectedRequest.financial_impact.administrative_cost || 0).toLocaleString('id-ID')}</p>
                        <p><strong>Net Loss/Gain:</strong> <span style={{ color: '#ff4d4f', fontWeight: 'bold' }}>Rp {((selectedRequest.financial_impact.loss_amount || 0) + (selectedRequest.financial_impact.tax_impact || 0) + (selectedRequest.financial_impact.administrative_cost || 0)).toLocaleString('id-ID')}</span></p>
                      </Card>
                    </Col>
                  </Row>
                </div>
              ...
              <Row gutter={[16, 16]}>
                <Col span={24}>
                  <div className="detail-section">
                    ...
                    <p><strong>Approved By:</strong> {selectedRequest.user_approval.approved_by}</p>
                    <p><strong>Approved Date:</strong> {selectedRequest.user_approval.approved_date}</p>
                    <p><strong>Notes:</strong> {selectedRequest.user_approval.notes}</p>
                  </div>
                </Col>
              </Row>
            )}
              ...
              <Row gutter={[16, 16]}>
                <Col span={24}>
                  <div className="detail-section">
                    ...
                    <p><strong>Approved By:</strong> {selectedRequest.finance_approval.approved_by}</p>
                    <p><strong>Approved Date:</strong> {selectedRequest.finance_approval.approved_date}</p>
                    <p><strong>Notes:</strong> {selectedRequest.finance_approval.notes}</p>
                    <p><strong>Financial Review:</strong> {selectedRequest.finance_approval.financial_review}</p>
                  </div>
                </Col>
              </Row>
            )}
                  ...
                  <div className="detail-section">
                    ...
                    >
                      <Row gutter={[16, 16]}>
                        <Col span={8}>
                          <Form as="form".Item
                            controlId="current_book_value"
                            label="Current Book Value (Rp)"
                          >
                        ...
                        <Col span={8}>
                          <Form as="form".Item
                            controlId="proposed_disposal_value"
                        ...
                        <Col span={8}>
                          <Form as="form".Item
                            controlId="loss_amount"
                            label="Loss Amount (Rp)"
                          >
                            <InputNumber style={{ width: '100%' }} />
                          </Form.Group>
                        </Col>
                      ...
                      <Row gutter={[16, 16]}>
                        ...
                        <Col span={12}>
                          <Form as="form".Item
                            controlId="administrative_cost"
                            label="Administrative Cost (Rp)"
                          >
                            <InputNumber style={{ width: '100%' }} />
                          </Form.Group>
                        </Col>
                      ...
                      <Form as="form".Item
                        controlId="financial_review_notes"
                        label="Financial Review Notes"
                      >
