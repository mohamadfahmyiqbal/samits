...
import { Form, Card, Table, Button, Input, Select, Row, Col, message, Modal, Alert, Tag, Progress, Steps } from 'antd';
import { 
  TruckOutlined, 
  EyeOutlined, 
  EditOutlined,
...
export default function DeliveryDistribution() {
  ...
  const mockDeliveryData = [
    {
      ...
      workflow: [
        { step: 'order_confirmed', status: 'completed', assignee: 'Sales Team', completed_date: '2024-03-29' },
        { step: 'preparing', status: 'completed', assignee: 'Warehouse Team', completed_date: '2024-03-30' },
    ...
    },
    {
      ...
      workflow: [
        { step: 'order_confirmed', status: 'completed', assignee: 'Sales Team', completed_date: '2024-03-30' },
    ...
    },
    {
      ...
      workflow: [
        { step: 'order_confirmed', status: 'completed', assignee: 'Sales Team', completed_date: '2024-03-27' },
        { step: 'preparing', status: 'completed', assignee: 'Warehouse Team', completed_date: '2024-03-28' },
        { step: 'in_transit', status: 'completed', assignee: 'Driver', completed_date: '2024-03-28' },
        { step: 'delivered', status: 'completed', assignee: 'Customer', completed_date: '2024-03-28' }
      ],
  ...
  useEffect(() => {
    let filtered = deliveryData.filter(delivery => {
      const matchesSearch = delivery.deliveryNumber.toLowerCase().includes(searchText.toLowerCase()) ||
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    ...
    try {
      setDeliveryData(prev => prev.map(delivery => {
        if (delivery.id === id) {
          const currentStepIndex = delivery.workflow.findIndex(step => step.status === 'in_progress');
  ...
  const getCurrentStep = (workflow) => {
    ...
    return inProgressStep ? workflow.findIndex(step => step.step === inProgressStep.step) : workflow.length - 1;
  };

  const pendingDeliveries = filteredData.filter(delivery => delivery.status === 'pending').length;
  const inProgressDeliveries = filteredData.filter(delivery => delivery.status === 'in_progress').length;
  const completedDeliveries = filteredData.filter(delivery => delivery.status === 'completed').length;
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (total, range) => 
              `${range[0]}-${range[1]} dari ${total} deliveries` }}
          rowClassName={(record) => {
            ...
            <Row gutter={[16, 16]}>
              <Col span={12}>
                <div className="detail-section">
                  ...
                  <p><strong>Status:</strong> <Tag color={getStatusColor(selectedDelivery.status)}>{selectedDelivery.status.replace('_', ' ').toUpperCase()}</Tag></p>
                  <p><strong>Priority:</strong> <Tag color={getPriorityColor(selectedDelivery.priority)}>{selectedDelivery.priority.toUpperCase()}</Tag></p>
              ...
              <Col span={12}>
                <div className="detail-section">
                  <h4>Schedule</h4>
                  <p><strong>Delivery Date:</strong> {selectedDelivery.deliveryDate}</p>
                  <p><strong>Estimated Time:</strong> {selectedDelivery.estimatedTime}</p>
                  {selectedDelivery.actualTime && (
                    <p><strong>Actual Time:</strong> {selectedDelivery.actualTime}</p>
                  )}
                ...
                <div className="detail-section">
                  ...
                  <Row gutter={[16, 16]}>
                    <Col span={12}>
                      ...
                      <p><strong>Contact:</strong> {selectedDelivery.customerContact}</p>
                      <p><strong>Phone:</strong> {selectedDelivery.customerPhone}</p>
                    </Col>
                    <Col span={12}>
                      <p><strong>Address:</strong> {selectedDelivery.customerAddress}</p>
                    </Col>
                  </Row>
                </div>
              </Col>
                ...
                <div className="detail-section">
                  ...
                  <table className="items-table">
                    ...
                    <tfoot>
                      ...
                      <tr>
                        <td colSpan="3" style={{ textAlign: 'right' }}>Delivery Fee:</td>
                        <td style={{ fontWeight: 'bold' }}>
                          Rp {selectedDelivery.deliveryFee.toLocaleString('id-ID')}
                        </td>
                      </tr>
                      <tr>
                        <td colSpan="3" style={{ textAlign: 'right', borderTop: '2px solid #f0f0f0' }}>Grand Total:</td>
                        <td style={{ fontWeight: 'bold', color: '#1890ff', borderTop: '2px solid #f0f0f0' }}>
                          Rp {(selectedDelivery.totalValue + selectedDelivery.deliveryFee).toLocaleString('id-ID')}
                        </td>
                      </tr>
                    </tfoot>
                  </table>
