...
import { Form, Card, Row, Col, Statistic, Table, Button, Modal, Input, Select, Tag, Space, message, Upload } from 'antd';
import { 
  FileTextOutlined,
  EyeOutlined,
  ReloadOutlined,
...
export default function FinanceDisposal() {
  ...
  useEffect(() => {
    let filtered = financeData.filter(asset => {
      const matchesSearch = asset.asset_name.toLowerCase().includes(searchText.toLowerCase()) ||
  ...
  const handleDisposalSubmit = async (values) => {
    ...
    try {
      ...
      setFinanceData(prev => prev.map(asset => 
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'pending_disposal': return 'orange';
    ...
    <div className="finance-disposal">
      ...
      >
        {selectedAsset && (
          ...
          >
            <Form as="form".Item controlId="asset_code" label="Asset Code">
              <Input disabled />
            </Form.Group>
            <Form as="form".Item controlId="asset_name" label="Asset Name">
              <Input disabled />
            </Form.Group>
            <Form as="form".Item
              controlId="disposal_reason"
            ...
            <Row gutter={[16, 16]}>
              ...
              <Col span={12}>
                <div className="detail-section">
                  ...
                  <p><strong>Purchase Value:</strong> Rp {selectedAsset.purchase_value.toLocaleString('id-ID')}</p>
                  <p><strong>Current Value:</strong> Rp {selectedAsset.current_value.toLocaleString('id-ID')}</p>
                  <p><strong>Book Value:</strong> Rp {selectedAsset.book_value.toLocaleString('id-ID')}</p>
              ...
              <Row gutter={[16, 16]}>
                <Col span={24}>
                  <div className="detail-section">
                    ...
                    <p><strong>Disposal Reason:</strong> {selectedAsset.disposal_reason}</p>
                    <p><strong>Disposal Date:</strong> {selectedAsset.disposal_date}</p>
                    <p><strong>Disposal Value:</strong> Rp {selectedAsset.disposal_value.toLocaleString('id-ID')}</p>
                    <p><strong>Loss Amount:</strong> Rp {selectedAsset.loss_amount.toLocaleString('id-ID')}</p>
                    <p><strong>Tax Impact:</strong> Rp {selectedAsset.tax_impact.toLocaleString('id-ID')}</p>
                    <p><strong>Net Loss:</strong> Rp {selectedAsset.net_loss.toLocaleString('id-ID')}</p>
                  </div>
                </Col>
              </Row>
            )}
