...
export default function Header({ side, handleUser = noop, handleSidebar }) {
 ...
 const userName = user?.nama || user?.name || "Nama Tidak Diketahui";
 const userDept = user?.dept || user?.position || "Dept Tidak Diketahui";
   ...
   <Navbar expand="lg" className="top-navbar px-3 py-2">
    <Container fluid className="d-flex align-items-center justify-content-between">
     ...
     <div className="d-flex align-items-center gap-2 gap-md-3">
      <Dropdown align="end">
       <Dropdown.Toggle variant="link" className="topbar-icon-btn" aria-label="Buka notifikasi">
      ...
      <Dropdown align="end">
       <Dropdown.Toggle variant="link" className="topbar-user-btn" aria-label="Buka menu pengguna">
        <div className="topbar-user-meta d-none d-md-flex">
         <span className="topbar-user-name">{loading ? "Memuat..." : userName}</span>
         <small className="topbar-user-dept">{loading ? "Profil pengguna" : userDept}</small>
        </div>
