// fe\src\layout\components\MenuItem.jsx
...
const MenuItem = memo(({
 ...
 const IconComponent = useMemo(() => {
  ...
  return typeof LucideIcons[iconName] === 'function'
 ...
 if (item.type === "dropdown" && item.items && item.items.length) {
  return (
   <div
    className={`menu-dropdown p-0 ${level === 0 ? 'border-b' : ''} transition-all duration-200 ${itemActive ? 'bg-blue-50 border-l-4 border-blue-500 dark:bg-blue-900/20' : ''
   ...
   >
    <div className="dropdown-toggle flex items-center p-3 cursor-pointer hover:bg-gray-100 hover:translate-x-1 dark:hover:bg-gray-800 rounded-lg">
     {IconComponent && <IconComponent className="w-4 h-4 mr-3 opacity-75" />}
     <span className="font-medium">{item.label}</span>

     <div className="ml-auto flex items-center gap-2">
      {badgeCount > 0 && (
       <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full min-w-[1.5rem] flex items-center justify-center">
 ...
 return (
  <div
   className={`menu-link p-3 cursor-pointer rounded-lg transition-all duration-200 ${itemActive
    ? 'bg-blue-500 text-white shadow-md border-l-4 border-blue-700'
    : 'hover:bg-gray-100 hover:translate-x-1 dark:hover:bg-gray-800'
    } ${isFocused ? 'ring-2 ring-blue-200 ring-opacity-50 shadow-lg' : ''}`}
   onClick={handleClick}
   role="menuitem"
   tabIndex={0}
