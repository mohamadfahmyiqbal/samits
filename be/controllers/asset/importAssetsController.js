...
const upsertItemNetwork = async (itemId, asset, transaction) => {
  ...
  const [updatedCount] = await ITItemNetwork.update(
    { hostname: hostname, updated_at: nowLiteral },
    {
      where: {
        it_item_id: itemId,
        is_primary: true,
 ...
 try {
  ...
  debugLog("[DEBUG] importAssets called, total:", assets?.length);
    ...
    if (!Array.isArray(assets) || assets.length === 0) {
      await safeRollback(transaction, "importAssets-empty-array");
  ...
  for (let i = 0; i < assets.length; i++) {
   try {
    ...
    results.errors.push({ row: i + 1, error: rowError.message });
        ...
        await safeRollback(transaction, `importAssets-row-${i + 1}`);
        logger.error(
  ...
  return res.status(200).json({
   ...
   results,
    ...
    await safeRollback(transaction, "importAssets-exception");
