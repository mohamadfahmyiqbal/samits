...
const getSubCategoryIdByType = async (asset, notFoundCategories = []) => {
  try {
    ...
    if (!subCategoryId || !categoryId) {
      ...
      if (subCategoryDirect?.sub_category_id && subCategoryDirect?.it_category_id) {
    ...
    if (!subCategoryId || !categoryId) {
      const fromCategory = await db.ITCategory.findOne({
        ...
        order: [[{ model: db.ITSubCategory, as: "subCategories" }, "sub_category_id", "ASC"]]
      });

      const fallbackSubCategoryId = fromCategory?.subCategories?.[0]?.sub_category_id || null;
    ...
    console.log(`[DEBUG] Found: ${subCategoryId} for "${typeName}", category_id: ${categoryId}`);
...
const upsertItemAssignment = async (itemId, asset) => {
  ...
  try {
    ...
    console.log(`[DEBUG][ASSIGN] asset=${noAsset || "-"} itemId=${itemId} nik="${nik}"`);
    ...
    if (!nik) {
      console.log(`[DEBUG][ASSIGN] skip insert assignment because nik is empty for asset=${noAsset || "-"}`);
      return;
    }
    ...
    console.log(`[DEBUG][ASSIGN] closed previous active assignments count=${closedCount} itemId=${itemId} nik=${nik}`);
    ...
    console.log(`[DEBUG][ASSIGN] activeSameNik count=${activeSameNik ? 1 : 0} itemId=${itemId} nik=${nik}`);
    ...
    if (!activeSameNik) {
      ...
      console.log(`[DEBUG][ASSIGN] inserted new assignment itemId=${itemId} nik=${nik}`);
    } else {
      console.log(`[DEBUG][ASSIGN] skip insert because active assignment already exists itemId=${itemId} nik=${nik}`);
    }
  } catch (error) {
    console.error(`[DEBUG][ASSIGN] abnormal for asset=${noAsset || "-"} itemId=${itemId}: ${error.message}`);
...
const upsertItemNetwork = async (itemId, asset) => {
  ...
  try {
    ...
    console.log(`[DEBUG][NETWORK] asset=${noAsset || "-"} itemId=${itemId} hostname="${hostname}"`);
    ...
    if (!hostname) {
      console.log(`[DEBUG][NETWORK] skip upsert network because hostname is empty for asset=${noAsset || "-"}`);
      return;
    }
    ...
    console.log(`[DEBUG][NETWORK] primary network updated count=${updatedCount} itemId=${itemId}`);
    ...
    if (!updatedCount) {
      ...
      console.log(`[DEBUG][NETWORK] inserted primary network itemId=${itemId} hostname="${hostname}"`);
    }
  } catch (error) {
    console.error(`[DEBUG][NETWORK] abnormal for asset=${noAsset || "-"} itemId=${itemId}: ${error.message}`);
  ...
  for (const candidate of candidates) {
    ...
    const classification = await ITClassification.findOne({
      where: sequelize.where(
        sequelize.fn('LOWER', sequelize.fn('LTRIM', sequelize.fn('RTRIM', sequelize.col('classification_name')))),
        normalized
      ),
...
const createNewAsset = async (asset, notFoundCategories) => {
  ...
  const tahunBeli = asset.tahunBeli ? String(asset.tahunBeli).slice(0, 4) : null;
  const period = tahunBeli ? `${tahunBeli}01` : null;
  
  const { subCategoryId, categoryId } = await getSubCategoryIdByType(asset, notFoundCategories);
  const classificationId = await getClassificationIdByAsset(asset);
  
  console.log(`[DEBUG] createNewAsset - noAsset: ${noAsset}, subCategoryId: ${subCategoryId}, categoryId: ${categoryId}, classificationId: ${classificationId}`);
  ...
  const newItem = await db.ITItem.create({
    ...
    asset_main_type_id: asset.asset_main_type_id ? Number(asset.asset_main_type_id) : null,
  });
...
export const importAssets = async (req, res) => {
  try {
    ...
    const results = { total: assets.length, created: 0, updated: 0, failed: 0, errors: [], notFoundCategories: [] };
    ...
    for (let i = 0; i < assets.length; i++) {
      try {
        ...
        const nik = extractNik(asset);
        const hostname = extractHostname(asset);
        console.log(`[DEBUG][IMPORT API] row=${i + 1} noAsset=${noAsset || "-"} nik="${nik}" hostname="${hostname}"`);
...
export const importAssetsFromExcel = async (req, res) => {
  try {
    ...
    const results = { total: 0, created: 0, updated: 0, failed: 0, errors: [], notFoundCategories: [] };
    ...
    for (const sheetName of selectedSheetNames) {
      ...
      if (!rawData || rawData.length < 5) {
        if (debugFlag) {
          console.log(`[DEBUG] Skip sheet "${sheetName}" karena format tidak valid`);
        }
        continue;
      }
      ...
      console.log(`[DEBUG] Sheet "${sheetName}" total rows: ${dataRows.length}`);
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        try {
          ...
          if (debugFlag) {
            console.log(`[DEBUG] Sheet "${sheetName}" Row ${rowNum} - noAsset: "${noAsset}", type: "${typeValue}"`);
          }
          ...
          console.log(`[DEBUG][IMPORT EXCEL] sheet=${sheetName} row=${rowNum} noAsset=${asset.noAsset || "-"} nik="${asset.nik}" hostname="${asset.hostname}"`);
          ...
          console.error(`[DEBUG] Sheet "${sheetName}" Row ${rowNum} - ERROR:`, rowError.message);
          results.failed++;
          results.errors.push({
            sheet: sheetName,
            row: rowNum,
