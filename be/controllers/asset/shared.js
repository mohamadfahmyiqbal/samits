...
export const resolveClassificationId = async (transaction) => {
 ...
 const getAssetGroup = await db.ITAssetGroup.findOne({
  where: {
   asset_group_name: transaction,
  },
 });

 const getSubCategory = await db.ITSubCategory.findOne({
  where: {
   sub_category_id: getAssetGroup?.sub_category_id || null,
  },
 });

 const getCategory = await db.ITCategory.findOne({
  where: {
   it_category_id: getSubCategory?.it_category_id || null,
  },
 });

 const result = ({
  asset_group_id: getAssetGroup?.asset_group_id || null,
  sub_category_id: getAssetGroup?.sub_category_id || null,
  it_category_id: getSubCategory?.it_category_id || null,
  asset_main_type_id: getCategory?.asset_main_type_id || null,
 });
