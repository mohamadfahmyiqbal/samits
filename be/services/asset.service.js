...
export class AssetService {
  ...
  static async resolveCategoryHierarchy(assetData, transaction = null) {
    ...
    const categoryId = await resolveCategoryId(assetData, subCategoryId, transaction);
  ...
  static async createITItem(payload, itItemId, uploaderNik, transaction) {
    ...
    await ITItem.create({
      ...
      asset_main_type_id: asset_main_type_id ? Number(asset_main_type_id) : null,
   ...
   * Buat audit log
   */
  static async createAuditLog(itItemId, assetNo, eventType, payload, req, uploaderNik, uploaderName, transaction) {
  ...
  static async uploadAssetDocuments(itItemId, files, uploaderNik, transaction) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const safeName = file.originalname?.replace(/[^\w.\-() ]+/g, '_').trim() || `doc-${Date.now()}.pdf`;
      const storedName = `${Date.now()}-${i + 1}-${crypto.randomUUID()}${ext}`;
      const absPath = path.join(destinationDir, storedName);
      const relPath = path.relative(path.resolve(process.cwd(), 'uploads'), absPath).replace(/\\/g, '/');
    ...
    if (documentRows.length > 0) {
      await AssetDocument.bulkCreate(documentRows, { transaction, returning: false });
    }
    return documentRows;
  }
  ...
  static async createAsset(payload, req, transaction) {
    ...
    const { subCategoryId, categoryId } = await this.resolveCategoryHierarchy(payload, transaction);
    ...
    await ITItem.create({
      ...
      classification_id: payload.classification_id ? Number(payload.classification_id) : 1,
      ...
      asset_group_id: payload.asset_group_id ? Number(payload.asset_group_id) : null,
    }, { transaction });
    ...
    await this.createStatusHistory(itItemId, payload.status || 'Active', transaction);
    
    // Audit log
    await this.createAuditLog(
      itItemId, noAsset, 'CREATED', payload, req, uploaderNik, uploaderName, transaction
    );
    
    // Documents
    const files = (req.files || []).filter(f => 
      f.fieldname === 'attachments' || /^attachments\[\d+\]$/.test(f.fieldname)
    );
  ...
  static async updateAsset(payload, req, foundAsset, transaction) {
    ...
    const updateData = {
      ...
      asset_group_id: payload.asset_group_id ? Number(payload.asset_group_id) : foundAsset.asset_group_id,
    };

    const ITItem = db.ITItem;
    await ITItem.update(updateData, { where: { it_item_id: itemId }, transaction });
